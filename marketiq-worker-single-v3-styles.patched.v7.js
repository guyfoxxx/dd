...
async function createWithdrawTicket(env, payload) {
  ...
  if (env.BOT_KV) {
    ...
    // keep for 90 days
    await env.BOT_KV.put(`wd:${ticket}`, JSON.stringify(data), { expirationTtl: 60 * 60 * 24 * 90 });
  }
  return ticket;
}
...
async function getSubRequest(env, ticket) {
  if (!env.BOT_KV) return null;
  const raw = await env.BOT_KV.get(`subreq:${ticket}`);
  if (!raw) return null;
  try { return JSON.parse(raw); } catch { return null; }
}
async function putSubRequest(env, ticket, data) {
  if (!env.BOT_KV) return;
  await env.BOT_KV.put(`subreq:${ticket}`, JSON.stringify(data), { expirationTtl: 60 * 60 * 24 * 180 });
}
...
async function handleUpdate(update, env) {
  try {
    ...
    if (text.startsWith("/setpromptstyle")) {
      ...
      await env.BOT_KV.put(`settings:analysis_prompt:${key}`, p);
    ...
    if (text === "/sublist") {
      ...
      if (!env.BOT_KV || !env.BOT_KV.list) return tgSendMessage(env, chatId, "⛔️ KV list در دسترس نیست.", mainMenuKeyboard(env));

      const res = await env.BOT_KV.list({ prefix: "subreq:", limit: 30 });
      const keys = res?.keys || [];
...
"), mainMenuKeyboard(env));
    ...
    if (text === BTN.SUB_BUY) {
      if (!env.BOT_KV) return tgSendMessage(env, chatId, "⚠️ سیستم اشتراک نیاز به BOT_KV دارد.", mainMenuKeyboard(env));
      if (st.subscription?.pendingTicket) {
        st.state = "sub_menu";
        await saveUser(userId, st, env);
...
async function adminApproveSub(env, ticket, adminFrom) {
  if (!env.BOT_KV) return { ok: false, error: "kv_required" };
  const req = await getSubRequest(env, ticket);
